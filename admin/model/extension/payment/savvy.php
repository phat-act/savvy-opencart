...
class ModelExtensionPaymentSavvy extends Model
{
    ...
    {
        $this->db->query("
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "savvy` (
                `savvy_id` INT(11) NOT NULL PRIMARY KEY AUTO_INCREMENT,
        ...
        $this->db->query("
            CREATE TABLE IF NOT EXISTS `". DB_PREFIX ."savvy_transaction` (
              `savvy_transaction_id` INT(11) NOT NULL PRIMARY KEY AUTO_INCREMENT,
    ...
    {
        $oldData = $this->db->query("SELECT * FROM `" . DB_PREFIX . "savvy`");
        ...
        foreach ($oldData->rows as $row) {
            ...
            if ($order['order_status_id'] != $this->config->get('payment_savvy_completed_status_id')) {
                continue;
            }
